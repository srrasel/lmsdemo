"use client";

import React, { useEffect, useRef, useState } from "react";
import { Navigation } from "swiper/modules";
import Pagination from "../Pagination";
import { Swiper, SwiperSlide } from "swiper/react";
import { useSection } from "./_hooks/useSection";
import useUtility from "@/app/_hooks/useUtility";
import ENDPOINTS from "@/lib/endpoints";
import { frontendImage, request } from "@/lib/helpers";
import { Image } from "react-bootstrap";
import CourseCard from "../CourseCard";
import SliderCourseSkeleton from "../_skeletons/SliderCourseSkeleton";

export const ShortCourses = ({ section }) => {
  const { content } = useSection(section);
  const [sortCourses, setShortCourses] = useState([]);
  const { trans } = useUtility();
  const swiperRef = useRef(null);

  // Unique ID for Swiper instance
  const swiperId = `short-courses-swiper-${Math.random()
    .toString(36)
    .substr(2, 9)}`;


  const [loading, setLoading] = useState(false)



  useEffect(() => {
      async function fetchShortCourses() {
          setLoading(true);
          try {
              const data = await request.get(ENDPOINTS.SHORT_COURSES);

              setShortCourses(data?.data?.data?.courses);
              setLoading(false);
          } catch (error) {
              setLoading(false);
              console.error("Error fetching categories:", error)
          }
      }
      fetchShortCourses()
  }, [])

  return (
    <section className="short-courses py-50">
      <div className="short-courses__shape">
        <Image
          src={frontendImage(content?.data_values?.shape, "short_courses")}
          alt="images"
        />
      </div>
      <div className="container">
        <div className="slider-top">
          <div className="section-heading style-left section-heading-dark">
            <h2 className="section-heading__title">
              {trans(content?.data_values?.heading)}
            </h2>
            <p className="section-heading__desc">
              {trans(content?.data_values?.subheading)}
            </p>
          </div>
          <div className="swipe-btn-wrapper">
              <div className={`swiper-button-prev ${swiperId}-prev`}></div>
              <div className={`swiper-button-next ${swiperId}-next`}></div>
            </div>
        </div>

        {loading ? (
          <div className="row">
            <SliderCourseSkeleton countCourse={4} />
          </div>
        ) : (
          <Swiper
            ref={swiperRef}
            modules={[Navigation, Pagination]}
            navigation={{
              nextEl: `.${swiperId}-next`,
              prevEl: `.${swiperId}-prev`,
            }}
        
            spaceBetween={20}
            breakpoints={{
              576: { slidesPerView: 2 },
              767: { slidesPerView: 2 },
              1024: { slidesPerView: 3 },
              1440: { slidesPerView: 4 },
            }}
          >
            {sortCourses?.map?.((course, index) => (
              <SwiperSlide key={index}>
                <CourseCard course={course} />
              </SwiperSlide>
            ))}
          </Swiper>
        )}
      </div>
    </section>
  );
};
