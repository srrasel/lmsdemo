...
export default function useTickets(noAuth = false) {
    ...
    const getTicket = async (ticket_number) => {
        try {
            ...
            const {
                data
            } = await request.get((noAuth ? ENDPOINTS.APP_VIEW_TICKET : ENDPOINTS.VIEW_TICKET) + '/' + ticket_number);
            return data.data;
        } catch (error) {
    ...
    const replyTicket = async (ticketId, values) => {
        try {
            setSubmitting(true);
            const { data } = await request.post((noAuth ? ENDPOINTS.APP_REPLY_TICKET : ENDPOINTS.REPLY_TICKET) + '/' + ticketId, getFormData(values));
            notifyToast(data);
            return data;
        } catch (error) {
            console.error('Error closing ticket:', error);
