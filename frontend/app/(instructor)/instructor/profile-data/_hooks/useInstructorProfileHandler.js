...
import { getCountries, notifyToast, request } from '@/lib/helpers';
...
const useInstructorProfileHandler = () => {
    ...
    const handleSubmit = async (values, { resetForm }) => {
        setLoading(true);
        try {
            const { data } = await request.post(ENDPOINTS.UPDATE_INSTRUCTOR_DATA, values);

            if (data.status === 'error') return notifyToast(data); 

            if(data.remark == "already_completed") {
                router.refresh();
                setCookie('is_logged_in_instructor', false);
                setCookie('access-token', null); 
                setCookie('instructor_data', null); 
...
export default useInstructorProfileHandler;
