        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-body">
                    ...
                    @if ($instructor->kv == Status::KYC_UNVERIFIED)
                        <div class="my-3">
                            <h6>@lang('Rejection Reason')</h6>
                            <p>{{ $instructor->kyc_rejection_reason }}</p>
                        </div>
                    @endif

                    @if ($instructor->kv == Status::KYC_PENDING)
                        <div class="d-flex flex-wrap justify-content-end mt-3">
                            ...
                            <button class="btn btn-outline--success confirmationBtn" data-question="@lang('Are you sure to approve this documents?')"
                                data-action="{{ route('admin.instructors.kyc.approve', $instructor->id) }}"><i
    ...
    <div id="kycRejectionModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.instructors.kyc.reject', $instructor->id) }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="alert alert-primary p-3">
                            @lang('If you reject these documents, the instructor will be able to re-submit new documents and these documents will be replaced by new documents.')
                        </div>
