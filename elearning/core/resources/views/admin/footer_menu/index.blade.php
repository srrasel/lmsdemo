                                        ...
                                        <td>
                                            <div class="button--group">
                                                <button class="btn btn-sm btn-outline--primary editMenu" 
                                                    data-id="{{ $menu->id }}" 
                                                    data-section_name="{{ $menu->section_name }}" 
                                                    data-name="{{ $menu->name }}" 
                                                    data-url="{{ $menu->url }}" 
                                                ...
                                                <button class="btn btn-sm btn-outline--danger confirmationBtn"
                                                    data-action="{{ route('admin.footer.menu.delete', $menu->id) }}"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="modalTitle">@lang('Add Footer Menu')</h4>
                ...
                <form method="post" action="{{ route('admin.footer.menu.store') }}">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label>@lang('Section Name')</label>
                            <input type="text" class="form-control" name="section_name" placeholder="@lang('e.g. Company, Support')" required>
                        </div>
                        <div class="form-group">
        ...
        <script>
            (function($) {
                ...
                $('.addMenu').on('click', function() {
                    var modal = $('#menuModal');
                    modal.find('.modal-title').text("@lang('Add Footer Menu')");
                    modal.find('form')[0].reset();
                    modal.find('form').attr('action', "{{ route('admin.footer.menu.store') }}");
                    modal.modal('show');
                });
                ...
                $('.editMenu').on('click', function() {
                    var modal = $('#menuModal');
                    var data = $(this).data();
                    modal.find('.modal-title').text("@lang('Edit Footer Menu')");
                    modal.find('input[name=section_name]').val(data.section_name);
                    ...
                    modal.find('form').attr('action', "{{ route('admin.footer.menu.update', '') }}/" + data.id);
                    modal.modal('show');
                });
            })(jQuery);
        </script>
