...
trait UserNotify
{
    public static function notifyToUser(){
        return [
            ...
            'enrolledCourses'       => 'Users Who Have Enrolled Courses',
            'notEnrolledCourses'       => 'Users Who Have Not Enrolled Any Courses',
            'completeCourses'       => 'Users Who Have Complete Courses',
            'HaveCertificate'      => 'User Who Have Get Certificate',
    ...
    public function scopeEnrolledCourses($query){
        return $query->whereHas('purchases', function($query){
            $query->where('payment_status', Status::PAYMENT_SUCCESS);
        });
    }

    public function scopeNotEnrolledCourses($query){
        return $query->whereDoesntHave('purchases', function($query){
            $query->where('payment_status', Status::PAYMENT_SUCCESS);
        });
    }

    public function scopeCompleteCourses($query){
        return $query->whereHas('completeCourses');
    }

    public function scopeHaveCertificate($query){
        return $query->whereHas('getCertificates');
    }
