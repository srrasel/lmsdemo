...
class DepositController extends Controller
{
    ...
    {
        if ($scope) {
            $deposits = Deposit::$scope()->with(['user', 'gateway'=> function ($gateway) {
                $gateway->withoutGlobalScopes();
            }]);
        }else{
            $deposits = Deposit::with(['user', 'gateway'=> function ($gateway) {
                $gateway->withoutGlobalScopes();
            }]);
        }
        ...
        if ($request->method) {
            if ($request->method != Status::GOOGLE_PAY) {
                $method = Gateway::where('alias',$request->method)->withoutGlobalScopes()->firstOrFail();
    ...
    {
        $deposit = Deposit::where('id', $id)->with(['user', 'gateway' => function ($gateway) {
            $gateway->withoutGlobalScopes();
        }])->firstOrFail();
