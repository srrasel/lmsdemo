...
class WithdrawalController extends Controller
{
    ...
    protected function withdrawalData($scope = null, $summary = false,$userId = null){
        ...
        if ($userId) {
            $withdrawals = $withdrawals->where('instructor_id',$userId);
        }
     
        $withdrawals = $withdrawals->searchable(['trx','user:username','instructor:username'])->dateFilter();
        ...
        if (!$summary) {
            return $withdrawals->with(['instructor','method'])->orderBy('id','desc')->paginate(getPaginate());
        }else{
            ...
            return [
                'data'=> $withdrawals->with(['instructor','method'])->orderBy('id','desc')->paginate(getPaginate()),
                'summary'=>[
    ...
    {
        $withdrawal = Withdrawal::where('id',$id)->where('status', '!=', Status::PAYMENT_INITIATE)->with(['instructor','method'])->firstOrFail();
    ...
    public function approve(Request $request)
    {
        ...
        $withdraw = Withdrawal::where('id',$request->id)->where('status',Status::PAYMENT_PENDING)->with('instructor')->firstOrFail();
        $withdraw->status = Status::PAYMENT_SUCCESS;
        $withdraw->admin_feedback = $request->details;
        $withdraw->save();

        notify($withdraw->instructor, 'WITHDRAW_APPROVE', [
    ...
    public function reject(Request $request)
    {
        ...
        $instructor = $withdraw->instructor;
        $instructor->balance += $withdraw->amount;
        $instructor->save();

        $transaction = new Transaction();
        $transaction->instructor_id = $withdraw->instructor_id;
        ...
        notify($instructor, 'WITHDRAW_REJECT', [
            'method_name' => $withdraw->method->name,
            'method_currency' => $withdraw->currency,
            'method_amount' => showAmount($withdraw->final_amount,currencyFormat:false),
            'amount' => showAmount($withdraw->amount,currencyFormat:false),
